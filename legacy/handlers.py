...
def create_git_blame_file(project_name, build_file_path, out_dir):
    """
    ...
    git_blame_output_file_path = \
        os.path.join(out_dir, project_name, GIT_BLAME_FILE_NAME)

    git_dir_file_path = os.path.join(build_file_dirname, ".git")

    LOG.info(f'pulling git blame for {project_name} at {git_dir_file_path}')
